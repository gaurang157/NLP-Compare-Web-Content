...
# Streamlit app
st.title("Compare 📰 Article Processor")
...
st.sidebar.link_button("NLP Compare Web Content (Selected 🎉 ✅)", "https://nlp-compare-web-content.streamlit.app/",use_container_width=True)
...
if st.button("Process URLs"):
    ...
    with col22:
        ...
        user_urls1 = [url1 for url1 in user_urls1 if url1.strip()]  # Remove empty URLs
        user_news_data1 = []

        if not user_urls1:
            st.write("Please enter valid URLs to proceed.")
        else:
            for url1 in user_urls1:
                news_item1 = process_url(url1)
                if news_item1 is not None:
                    user_news_data1.append(news_item1)

            if not user_news_data1:
                st.write("No valid URLs provided. Please enter valid URLs.")
            else:
                english_texts1 = []
                hindi_texts1 = []

                for news_item1 in user_news_data1:
                    if news_item1['Language'] == 'en':
                        english_texts1.append(news_item1['Article Text'])
                    elif news_item1['Language'] == 'hi':
                        hindi_texts1.append(news_item1['Article Text'])
                ...
                if english_texts1:
                    ...
                    # Create word cloud or bar chart for English english_text
                    # ...
                    english_text1 = '\n'.join(news_item1['Article Text'] for news_item1 in user_news_data1)
                    # from setup import analyzer, emotion_analyzer, hate_speech_analyzer
                    # from transformers import AddedToken
                    ...
                    # emotion1 = analyzers[1].predict(english_text)
                    # hate_speech1 = analyzers[2].predict(english_text)
                    # from setup import analyzer, emotion_analyzer, hate_speech_analyzer #IMP
                    # from transformers import AddedToken
                    ...
                    def get_analyzers1():
                        from setup import analyzer, emotion_analyzer, hate_speech_analyzer
                        return analyzer, emotion_analyzer, hate_speech_analyzer
                    analyzers1 = get_analyzers1()
                    sentiment11 = analyzers1[0].predict(english_text1)
                    emotion11 = analyzers1[1].predict(english_text1)
                    hate_speech11 = analyzers1[2].predict(english_text1)




                    TOInews1 = re.sub("[^A-Za-z" "]+", " ", english_text1).lower()
                    ...
                    # Perform english_text preprocessing without removing full stops
                    sen1 = re.sub(pattern1, " ", english_text1).lower()
                    ...
                    st.subheader("Emotion Analysis")
                    st.write(emotion11)
                    emotion_output1 = emotion11.output
                    probas_emotion1 = emotion11.probas
                    others1 = probas_emotion1.get("others")
                    joy1 = probas_emotion1.get("joy")
                    disgust1 = probas_emotion1.get("disgust")
                    fear1 = probas_emotion1.get("fear")
                    sadness1 = probas_emotion1.get("sadness")
                    surprise1 = probas_emotion1.get("surprise")
                    anger1 = probas_emotion1.get("anger")
                    ...
                    # Extract emotion labels and probabilities
                    emotions1112 = emotions1011.keys()
                    probabilities1112 = emotions1011.values()
                    
                    # Create a bar plot
                    with _lock:
                        plt.figure(figsize=(8, 4))
                        plt.bar(emotions1112, probabilities1112, color=['blue', 'green', 'red', 'purple', 'orange', 'pink'])
                        plt.xlabel("Emotion")
                        plt.ylabel("Probability")
                        plt.title("Emotion Probabilities")
                        plt.ylim(0, 1)  # Set the y-axis limit from 0 to 1
                    ...
                    st.write("Emotion Output:", emotion_output1)
                    st.write("Probas Emotion:")
                    st.write("Others:", others1)
                    st.write("Joy:", joy1)
                    st.write("Disgust:", disgust1)
                    st.write("Fear:", fear1)
                    st.write("Sadness:", sadness1)
                    st.write("Surprise:", surprise1)
                    st.write("Anger:", anger1)
                    # Show the plot
                    ...
                    hate_speech1_output1 = hate_speech11.output
                    probas_hate_speech11 = hate_speech11.probas
                    # Extract the values
                    hateful1 = probas_hate_speech11.get("hateful")
                    targeted1 = probas_hate_speech11.get("targeted")
                    aggressive1 = probas_hate_speech11.get("aggressive")
                    ...
                    # Extract hate speech labels and probabilities
                    labels12 = hate_speech12.keys()
                    probs12 = hate_speech12.values()
                    
                    # Create a bar plot
                    with _lock:
                        plt.figure(figsize=(8, 4))
                        plt.bar(labels12, probs12, color=['red', 'green', 'blue'])
                        plt.xlabel("Category")
                        plt.ylabel("Probability")
                        plt.title("Hate Speech Probabilities")
                        plt.ylim(0, 1)  # Set the y-axis limit from 0 to 1
                        ...
                        plt.show()
                        st.pyplot(plt, use_container_width=True)
                        st.write("Hate Speech Output:", hate_speech1_output1)
                    
                    st.write("Probas:")
                    st.write("Hateful:", hateful1)
                    st.write("Targeted:", targeted1)
                    st.write("Aggressive:", aggressive1) 

                    for idx1, news_item1 in enumerate(user_news_data1, start=1):
                        st.subheader(f"Article {idx1}: {news_item1['Title']}")
                        st.write("Article Text:")
                        st.write(news_item1['Article Text'])
                        st.write("Article Summary:")
                        st.write(news_item1['Article Summary'])
                        st.write("Article Keywords:")
                        st.write(', '.join(news_item1['Article Keywords']))
                        st.markdown("---")    
                    st.balloons()
